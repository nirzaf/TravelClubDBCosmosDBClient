...
namespace CosmosDBClientSample
{
    internal class Program
    {
        ...
        private static async Task AddItemsToContainerAsync(Container container)
        {
            ...
            try
            {
                await container.ReadItemAsync<Destination>(destinationToAdd.Id,
                    new PartitionKey(destinationToAdd.DestinationName));
            }
            catch (CosmosException ex) when (ex.StatusCode == System.Net.HttpStatusCode.NotFound)
            {
                // Create an item in the container representing the Andersen family. Note we provide the value of the partition key for this item, which is "Andersen"
                await container.CreateItemAsync<Destination>(destinationToAdd,
                    new PartitionKey(destinationToAdd.DestinationName));
            }
        }
    }
